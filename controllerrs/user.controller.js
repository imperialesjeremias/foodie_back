  ...
  register: async (req, res) => {
    try {
     const { firstName, lastName, email, password } = req.body;                             // recibimos los datos desde el body
        !firstName || !lastName || !email || !password && res.status(400).json({ message: "Faltan campos por llenar" }); // validamos que los campos no esten vacíos
        ...
        const user = new User({ firstName, lastName, email, password });                // creamos el usuario
